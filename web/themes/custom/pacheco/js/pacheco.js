...
(function ($, Drupal) {
  ...
  Drupal.behaviors.pacheco = {
    attach: function (context, settings) {
      ...
      *********************/
      //FOTOS
      $('#lightgallery-start-fotos, .field-name-field-galeria img', context).once('pacheco').on('click touchstart',function(e){
        e.preventDefault();
        $(".gallery-lightgallery .lightgallery-wrapper:nth-child(2) a > img").first().trigger("click");
      });
      //ESQUEMAS
      $('#lightgallery-start-esquemas, .field-name-field-galeria-esquemas img', context).once('pacheco').on('click touchstart',function(e){
        e.preventDefault();
        $(".gallery-lightgallery .lightgallery-wrapper:nth-child(3) a > img").first().trigger("click");
      });
      //PLANOS
      $('#lightgallery-start-planos, .field-name-field-galeria-planos img', context).once('pacheco').on('click touchstart',function(e){
        e.preventDefault();
        $(".gallery-lightgallery .lightgallery-wrapper:nth-child(4) a > img").first().trigger("click");
      });
      //REFERENTES
      $('#lightgallery-start-referentes, .field-name-field-galeria-referentes img', context).once('pacheco').on('click touchstart',function(e){
        e.preventDefault();
